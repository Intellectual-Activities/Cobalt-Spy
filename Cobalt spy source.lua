...
local function WrapCodeInActor(Code: string, ShouldWrap: boolean)
	...
	return string.format(WrapperActorCode, IndentCode(Code, 2))
end
...
local LucideIcons = loadstring(game:HttpGet(
	"https://raw.githubusercontent.com/Intellectual-Activities/lucide-roblox-icons/refs/heads/main/Lucide%20source.lua"
))()

local function GetLocalAsset(path: string): string?
	if not (isfile and isfile(path)) then
		return nil
	end

	if getsynasset then
		local assetSuccess, asset = pcall(getsynasset, path)
...
function Icons.GetIcon(iconName: string): Icon?
	...
	if type(iconName) == "string" then
		...
		if iconName:find("/") or iconName:find("\\") or iconName:match("%.[%a%d]+$") then
			table.insert(candidates, iconName)
		end

		table.insert(candidates, `lucideReborn-icons/{iconName}.png`)

		for _, path in ipairs(candidates) do
			local asset = GetLocalAsset(path)
			if asset then
				return {
					Url = asset,
					Id = 0,
	...
	if not LucideIcons or type(LucideIcons.GetAsset) ~= "function" then
		return
	end

	local RemoteSuccess, Icon = pcall(LucideIcons.GetAsset, iconName)
...
function Icons.SetIcon(imageInstance: ImageLabel | ImageButton, iconName: string)
	...
	if iconString:match("^%d+$") then
		...
		imageInstance.Image = "rbxassetid://" .. iconString
		imageInstance.ImageRectOffset = Vector2.zero
		imageInstance.ImageRectSize = Vector2.zero
		return
	end

	if iconString:match("^rbxassetid://") or iconString:match("^rbxasset://") or iconString:match("^https?://") then
		imageInstance.Visible = true
		imageInstance.Image = iconString
		imageInstance.ImageRectOffset = Vector2.zero
		imageInstance.ImageRectSize = Vector2.zero
		return
	end

	local Icon: Icon? = Icons.GetIcon(iconString)
	if not Icon then
		imageInstance.Visible = false
		return
	end
